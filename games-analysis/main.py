...
plt.scatter(df_main["first_release_date"],
df_main["user_rating_value"], color = "red", label = "User Ratings")
...
df_follow = df_follow.rename(columns = { "follow__count": "total_follows" })

df_hype = df_main.groupby(["genre_name"]) ["hype_count"].sum().reset_index()

df_hype = df_hype.rename(columns = { "hyoe_count": "total_hypes"})

plt.bar(df_follow.index - BAR_WIDTH / 2, df_follow["total_follows"], coloe = "blue", label = "Number of Follows", width = BAR_WIDTH)
